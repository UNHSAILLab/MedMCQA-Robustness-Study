...
\begin{table}[t]
\centering
\caption{Sample model responses illustrating key failure modes. All examples from MedGemma-4B on MedMCQA.}
...
\addlinespace
CoT output & ``A) Tuberculosis: Oral TB is rare in children\ldots B) Lichen planus: less common in infants\ldots'' \textit{(analyzes each option, then selects \texttt{A})} \quad $\times$ \\
\midrule
\multicolumn{2}{l}{\textbf{Example 2: Option order flip} (Anatomy, correct answer: B $\to$ A after shuffle)} \\
\midrule
Original & A)~Sphenopalatine, B)~Anterior ethmoidal, C)~Greater palatine, D)~Septal branch. Model answers \textbf{B}. \quad $\checkmark$ \\
\addlinespace
Shuffled & A)~Anterior ethmoidal, B)~Septal branch, C)~Sphenopalatine, D)~Greater palatine. Model answers \textbf{B}. \quad $\times$ \\
\addlinespace
 & Same question, same knowledge. The correct content moved from B to A, but the model still picks position B. \\
\bottomrule
\end{tabular}
\end{table}
